...
uint8_t button_getButton() {
    ...
    int x;
    for(x=3; x>=0 ;x--){     //rightmost loop
    //for(x=0; x<=3 ;x++){        //leftmost loop
        if (((GPIO_PORTE_DATA_R >> x) & 1) == 0){        //shift data by x, if zero, its pressed so it would return new button pressed
            return(x+1);       //return left most button pressed
        }
    }
